...
/* SPI timeout */
#define RA_SPI_TIMEOUT_MS          1000

/* Default CS timing values     */
#define RA_SPI_CS_SETUP_DELAY          2         /* Default CS setup delay cycles */
#define RA_SPI_CS_HOLD_DELAY           2         /* Default CS hold delay cycles */
#define RA_SPI_CS_NEGATION_DELAY       0         /* Default CS negation delay cycles */
...
static int ra_spi_get_transfer_size(struct ra_spi_priv_s *priv)
{
  ...
  uint32_t spb = spcmd0 & RA_SPI_SPCMD_SPB_MASK;

  /* Decode SPB field value to bit count */
  switch (spb)
    {
      case RA_SPI_SPCMD_SPB_4:
      case RA_SPI_SPCMD_SPB_8:
        return 1;  /* 4-bit or 8-bit transfers */

      case RA_SPI_SPCMD_SPB_16:
        return 2;  /* 16-bit transfers */

      case RA_SPI_SPCMD_SPB_20:
      case RA_SPI_SPCMD_SPB_24:
      case RA_SPI_SPCMD_SPB_32:
...
static void ra_spi_setmode(struct spi_dev_s *dev, enum spi_mode_e mode)
{
  ...
  uint16_t spcmd0 = ra_spi_getreg32(priv, RA_SPI_SPCMD0_OFFSET);
...
static void ra_spi_bus_initialize(struct ra_spi_priv_s *priv)
{
  ...
  if (priv->config->master_mode)
    {
      ...
      uint32_t setup = RA_SPI_CS_SETUP_DELAY;
      uint32_t neg   = RA_SPI_CS_NEGATION_DELAY;
      uint32_t hold  = RA_SPI_CS_HOLD_DELAY;
  ...
  if (priv->config->dev_config == NULL ||
      (priv->config->num_cs > 0 && priv->config->dev_config[0].cs_type == RA_SPI_CS_CLK_SYS))
    {
                                 ...
                                 bool selected)
{
    ...
    /* Assert CS */
    if (dev_config->cs_type == RA_SPI_CS_GPIO)
      {
