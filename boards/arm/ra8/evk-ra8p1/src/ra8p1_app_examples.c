/****************************************************************************
 * boards/arm/ra8/evk-ra8p1/src/ra8p1_app_examples.c
 *
 * Unified example wrappers that provide a single entry point
 * ra8p1_<app>_example() for each board example to simplify calls
 * from ra8p1_bringup().
 ****************************************************************************/

#include <nuttx/config.h>
#include <syslog.h>

#include "evk-ra8p1.h"

#ifdef CONFIG_RA8P1_SPI_LOOPBACK_EXAMPLE
int ra8p1_spi_loopback_example(void)
{
  int ret = ra8p1_spi_loopback_init();
  if (ret < 0)
    {
      syslog(LOG_INFO, "Test initialization failed: %d\n", ret);
      return ret;
    }

  return ra8p1_spi_loopback_main(0, NULL);
}
#endif

#ifdef CONFIG_RA8P1_SPI_MASTERSLAVE_EXAMPLE
int ra8p1_spi_masterslave_example(void)
{
  int ret = ra8p1_spi_masterslave_init();
  if (ret < 0)
    {
      return ret;
    }

  return ra8p1_spi_masterslave_test();
}
#endif

#ifdef CONFIG_RA8P1_ADC_BMS_EXAMPLE
int ra8p1_adc_bms_example(void)
{
  int ret = ra8p1_adc_bms_init();
  if (ret < 0)
    {
      return ret;
    }

  return ra8p1_adc_bms_main(0, NULL);
}
#endif

#ifdef CONFIG_RA8P1_ADC_BMS_EXAMPLE
int ra8p1_adc_example(void)
{
  int ret = ra8p1_adc_init();
  if (ret < 0)
    {
      return ret;
    }

  return ra8p1_adc_bms_main(0, NULL);
}
#endif

#ifdef CONFIG_RA8P1_CODE_FLASH_EXAMPLE
int ra8p1_code_flash_example(void)
{
  int ret = ra8p1_code_flash_init();
  if (ret < 0)
    {
      return ret;
    }

  /* Run any self-tests if available */
  ra8p1_code_flash_test();
  return 0;
}
#endif

#ifdef CONFIG_RA8P1_DATA_FLASH_EXAMPLE
int ra8p1_data_flash_example(void)
{
  int ret = ra8p1_data_flash_init();
  if (ret < 0)
    {
      return ret;
    }

  /* Run test routine if present */
  ra8p1_data_flash_test();
  return 0;
}
#endif

#ifdef CONFIG_RA8P1_PWM_ESCS_EXAMPLE
int ra8p1_gpt_escs_example(void)
{
  int ret = ra8p1_gpt_escs_init();
  if (ret < 0)
    {
      return ret;
    }

  return ra8p1_gpt_escs_main(0, NULL);
}
#endif

#ifdef CONFIG_RA8P1_GPS_EXAMPLE
int ra8p1_gps_example(void)
{
  int ret = ra8p1_gps_init();
  if (ret < 0)
    {
      return ret;
    }

  return ra8p1_gps_main(0, NULL);
}
#endif

#ifdef CONFIG_RA8P1_SBUS_EXAMPLE
int ra8p1_sbus_example(void)
{
  int ret = ra8p1_sbus_init();
  if (ret < 0)
    {
      return ret;
    }

  return ra8p1_sbus_main(0, NULL);
}
#endif

#ifdef CONFIG_RA8P1_I2C_ACC_EXAMPLE
int ra8p1_i2c_acc_example(void)
{
  int ret = ra8p1_i2c_acc_init();
  if (ret < 0)
    {
      return ret;
    }

  return ra8p1_i2c_acc_main(0, NULL);
}
#endif

#ifdef CONFIG_RA8P1_I2C_GY912_EXAMPLE
int ra8p1_i2c_gy912_example(void)
{
  int ret = ra8p1_i2c_gy912_init();
  if (ret < 0)
    {
      return ret;
    }

  return ra8p1_i2c_gy912_main(0, NULL);
}
#endif

#ifdef CONFIG_RA8P1_RUST_EXAMPLE
int ra8p1_rust_example(void)
{
  /* Existing thread init is the entry point */
  return ra8p1_rust_sample_init();
}
#endif

#ifdef CONFIG_RA8P1_OSPI_EXAMPLE
int ra8p1_ospi_example(void)
{
  int ret = ra8p1_ospi_test_init();
  if (ret < 0)
    {
      syslog(LOG_ERR, "OSPI test init failed: %d\n", ret);
      return ret;
    }

  return ra8p1_ospi_test_main(0, NULL);
}
#endif

#ifdef CONFIG_RA8P1_GPT_PWM_EXAMPLE
int ra8p1_gpt_pwm_example(void)
{
  int ret = ra8p1_gpt_pwm_initialize();
  if (ret < 0)
    {
      syslog(LOG_ERR, "GPT PWM initialization failed: %d\n", ret);
      return ret;
    }

  return ra8p1_gpt_pwm_main(0, NULL);
}
#endif


/* Run all enabled application examples. Returns 0 on success or the
 * last non-zero error code from any example.
 */
int ra8p1_app_examples(void)
{
  int ret = 0;
  int last_err = ret;

#ifdef CONFIG_RA8P1_GPT_PWM_EXAMPLE
  ret = ra8p1_gpt_pwm_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "GPT PWM example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_ADC_BMS_EXAMPLE
  ret = ra8p1_adc_bms_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "ADC BMS example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_CODE_FLASH_EXAMPLE
  ret = ra8p1_code_flash_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "Code Flash example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_DATA_FLASH_EXAMPLE
  ret = ra8p1_data_flash_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "Data Flash example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_PWM_ESCS_EXAMPLE
  ret = ra8p1_gpt_escs_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "ESCs example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_GPS_EXAMPLE
  ret = ra8p1_gps_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "GPS example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_SBUS_EXAMPLE
  ret = ra8p1_sbus_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "SBUS example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_I2C_GY912_EXAMPLE
  ret = ra8p1_i2c_gy912_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "I2C GY-912 example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_I2C_ACC_EXAMPLE
  ret = ra8p1_i2c_acc_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "I2C ACC example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_SPI_LOOPBACK_EXAMPLE
  ret = ra8p1_spi_loopback_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "SPI Loopback example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_SPI_MASTERSLAVE_EXAMPLE
  ret = ra8p1_spi_masterslave_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "SPI Master/Slave example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_SPI_GY912_EXAMPLE
  ret = ra8p1_spi_gy912_init();
  if (ret < 0)
    {
      syslog(LOG_ERR, "SPI GY-912 init failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_RUST_EXAMPLE
  ret = ra8p1_rust_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "Rust example failed: %d\n", ret);
      last_err = ret;
    }
#endif

#ifdef CONFIG_RA8P1_OSPI_EXAMPLE
  ret = ra8p1_ospi_example();
  if (ret < 0)
    {
      syslog(LOG_ERR, "OSPI example failed: %d\n", ret);
      last_err = ret;
    }
#endif

  return last_err;
}
